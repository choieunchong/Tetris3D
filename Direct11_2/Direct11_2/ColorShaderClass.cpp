...
{
	// 정점 및 픽셀 쉐이더를 초기화합니다.
	return InitializeShader(device, hwnd, L"../Direct11_2/color.vs", L"../Direct11_2/color.ps");
}
...
bool ColorShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd,const WCHAR* vsFilename,const WCHAR* psFilename)
{
	...
	// 일치해야 하는 이유 : ModelClass와 셰이더의 VertexType 구조가 일치하지 않는다면,
	//데이터의 구조와 형식이 서로 다르기 때문에 셰이더는 올바르게 데이터를 해석할 수 없습니다
	//D3D11_INPUT_ELEMENT_DESC[2] 위치 정보와 색상에 대한 정보를 담기위해
...
bool ColorShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix)
{
	...
	// 상수 버퍼에 행렬을 복사합니다.
	// 3가지로 복사하는 이유
	//객체의 위치, 회전 및 크기 월드 행렬 개체를 월드 공간에 배치 하기 위해
	// 뷰 카메라의 시점 변환
	// 프로젝션 3D 공간을 2D 화면에 투영 하기 위해 깊이 효과와 원근감을 처리 하기 위해
	dataPtr->world = worldMatrix;
	dataPtr->view = viewMatrix;
	dataPtr->projection = projectionMatrix;

	// 상수 버퍼의 잠금을 풉니다.
	// 잠금을 푸는 이유는 업데이트 된 데이터가 상수 버퍼에 적용되고 사용되기 위해
	// 잠금을 풀기 전까지 GPU에서 데이터를 사용 할 수 없다.
